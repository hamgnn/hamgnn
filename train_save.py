...
def train(args):
    ...
    with open('data_'+args.dataset+args.model+str(args.num_layers)+'.pkl', 'wb') as f1:
        pickle.dump(data,f1)

    f1.close()

    # adj_save = data['adj_train']
    # np.save('adj_airport.npy',adj_save)
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % args.eval_freq == 0:
            ...
            if model.has_improved(best_val_metrics, val_metrics):
                ...
                # save_out = model.decode_save(embeddings, data['adj_train_norm'])
                best_emb = embeddings.cpu()
                # save_emb = model.encoder.encode_save(data['features'], data['adj_train_norm'])
                # save_out =save_out.cpu()
                if args.save:
    ...
    if not best_test_metrics:
        ...
        with torch.no_grad():
            ...
            # save_emb = model.encoder.encode_save(data['features'], data['adj_train_norm'])
            # save_out = model.decode_save(best_emb, data['adj_train_norm'])
    ...
    if args.save:
        ...
        # np.save(os.path.join(save_dir, 'saveemb_' + str(args.dataset) + args.odemap + str(args.cuda) + str(
        #     args.task) + '.npy'), save_emb.cpu().detach().numpy())
        # np.save(os.path.join(save_dir, 'saveout_' + str(args.dataset) + args.odemap + str(args.cuda) + str(
        #     args.task) + '.npy'), save_out.cpu().detach().numpy())
        ...
        torch.save(model.state_dict(), os.path.join(save_dir, 'model_'+str(args.dataset)+args.odemap+str(args.cuda)+str(args.task)+args.model+str(args.num_layers)+'.pth'))
        torch.save(model.encoder, os.path.join(save_dir, 'model_' + str(args.dataset) + args.odemap + str(args.cuda) + str(args.task)+args.model+str(args.num_layers) + 'model.pth'))
