...
class myLinear(nn.Module):
    def __init__(self, size_in):
        ...
        if args.acth2=='act2':
            self.act = my_act(act2)
        if args.acth2=='act3':
            self.act = my_act(act3)
        if args.acth2=='rehu':
            self.act = ReHU(d=0.1)
        ...
        # -- 
        feat_h2_scale = args.feat_h2_scale # default 8
        
        w1_z = torch.Tensor(self.dim*feat_h2_scale, self.dim*2)
        self.w1_z = nn.Parameter(w1_z)
        b1 = torch.Tensor(self.dim*feat_h2_scale)
        self.b1 = nn.Parameter(b1)
        
        w2_z = torch.Tensor(self.dim*2, self.dim*feat_h2_scale)
