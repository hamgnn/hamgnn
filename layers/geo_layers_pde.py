...
class ODEBlock(nn.Module):
    ...
    def __init__(self, odefunc,out_features, dropout, use_att, n_nodes,odemap,args,adj,time):
        super(ODEBlock, self).__init__()
        self.odefunc = odefunc(out_features, dropout, use_att, n_nodes,odemap,args,adj,)
...
class GeoGraphConvolution(nn.Module):
    ...
    def forward(self, input):
        ...
        self.odeblock_exp = ODEBlock(self.agg_ode,self.out_features, self.dropout, self.use_att, self.n_nodes, self.odemap,self.args,adj,[0, 1])
        h = self.odeblock_exp(x)
...
class GeoAgg_ODE(Module):
    """
    Hyperbolic aggregation layer.
    """

    def __init__(self,  in_features, dropout, use_att, n_nodes,odemethod,args,adj):
        super(GeoAgg_ODE, self).__init__()
    ...
    def forward(self,t, x):
        ...
        if 'learn' in self.odemethod :
            ...
            drop_weight = self.weight.to(self.args.cuda)
            # print("xt",xt)
            # print("drop_weight", drop_weight)
            vt = xt @ drop_weight.transpose(-1, -2)
            xt = torch.hstack([xt, vt])
            out = self.odefunc(xt, )
            out = out[..., 0:int(self.in_features)]
        ...
        x_tangent = out
        if self.use_att:
            input_att = x_tangent, self.adj
            support_t ,_ = self.att(input_att)
            # support_t = self.att(x_tangent, from_scipy_sparse_matrix(adj)[0].to(self.args.cuda))

        else:
            support_t = torch.spmm(self.adj , x_tangent)
            # support_t = x_tangent
