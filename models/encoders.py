...
import layers.geo_layers_v1 as geo_layers
#
# import layers.geo_layers_pde as geo_layers

from layers.layers import GraphConvolution, Linear, get_dim_act
from layers.att_layers import SpGraphAttentionLayer,GraphAttentionLayer
import utils.math_utils as pmath

import scipy.sparse as sp
import dgl
# from layers.ode_map import Connection_g,ODEfunc, Connectionnew, Connection_gnew,Connection,Connection_ricci,Connection_riccilearn,Connection_v5,Connection_v5new
...
class GeoGCN(Encoder):
    """
    Geo-GCN.
    """

    def __init__(self, c, args):
        super(GeoGCN, self).__init__(c)
        self.manifold = getattr(manifolds, args.manifold)()
        # assert args.num_layers > 1
        dims, acts, self.curvatures = geo_layers.get_dim_act_curv(args)
        ...
        for i in range(len(dims) - 1):
            ...
            act = acts[i]
            geognn_layers.append(
                    geo_layers.GeoGraphConvolution(
                            in_dim, in_dim, args.dropout, act, args.bias, args.use_att, args.local_agg,args.cuda,args.n_nodes,args
                    )
            )
        # self.linearlayer = nn.Linear(in_features=dims[0]-1,out_features=dims[1]-1,bias=False)
        self.linearlayer1 = nn.Linear(in_features=args.feat_dim , out_features=args.dim)
        self.layers = nn.Sequential(*geognn_layers)
        self.encode_graph = True
    ...
    def encode(self, x, adj,):
        x_embedding = self.linearlayer1(x)

        return super(GeoGCN, self).encode(x_embedding, adj,)
